...
async function load(){var r=await fetch(A+'/campaigns').then(function(r){return r.json()});items=r.campaigns||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.title||'').toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)||(i.status||'').toLowerCase().includes(q)||(i.end_date||'').toLowerCase().includes(q)});
...
if(i.goal_amount)h+='<span>'+i.goal_amount+'</span>';
if(i.raised_amount)h+='<span>'+i.raised_amount+'</span>';
if(i.backer_count)h+='<span>'+i.backer_count+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/campaigns/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{title:"",description:"",goal_amount:0,raised_amount:0,backer_count:0,status:"",end_date:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' GRUBSTAKE</h2>';
...
h+='<div class="fr"><label>Goal Amount</label><input id="f-goal-amount" type="number" value="'+(i.goal_amount||0)+'"></div>';
h+='<div class="fr"><label>Raised Amount</label><input id="f-raised-amount" type="number" value="'+(i.raised_amount||0)+'"></div>';
h+='<div class="fr"><label>Backer Count</label><input id="f-backer-count" type="number" value="'+(i.backer_count||0)+'"></div>';
...
async function submit(){var body={title:document.getElementById('f-title').value.trim(),description:document.getElementById('f-description').value.trim(),goal_amount:parseInt(document.getElementById('f-goal-amount').value)||0,raised_amount:parseInt(document.getElementById('f-raised-amount').value)||0,backer_count:parseInt(document.getElementById('f-backer-count').value)||0,status:document.getElementById('f-status').value.trim(),end_date:document.getElementById('f-end-date').value.trim()};
if(editId){await fetch(A+'/campaigns/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/campaigns',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
